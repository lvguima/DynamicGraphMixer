...
def main() -> None:
    parser = argparse.ArgumentParser(description="v3 ablation experiments (A0/A1/A2/A3).")
    parser.add_argument(
        "--stage",
        choices=["A0", "A1", "A2", "A3", "all"],
        default="all",
        help="Which ablation stage to run.",
    )
    parser.add_argument(
        "--graph_log_interval",
        type=int,
    ...
    if args.data.lower() == "weather":
        ...
        features = "M"
        target = "OT"
        freq = args.freq if args.freq else "h"
    ...
    if args.stage in ("A0", "all"):
        exp_id = f"v3_A0_{dataset_tag}"
        model_id = f"DGmix_v3_DS_SMGP_{dataset_tag}_96_96_A0"
        run_exp(args.python_bin, exp_id, model_id, base_args, [*map_args], args.dry_run)

    if args.stage in ("A1", "all"):
        exp_id = f"v3_A1_noGraphProp_{dataset_tag}"
        model_id = f"DGmix_v3_DS_SMGP_{dataset_tag}_96_96_A1"
        run_exp(
            args.python_bin,
            exp_id,
            model_id,
            base_args,
            [*map_args, "--gate_init", "-20"],
            args.dry_run,
        )

    if args.stage in ("A2", "all"):
        exp_id = f"v3_A2_noSMGP_{dataset_tag}"
        model_id = f"DGmix_v3_DS_noSMGP_{dataset_tag}_96_96_A2"
        run_exp(
            args.python_bin,
            exp_id,
            model_id,
    ...
    if args.stage in ("A3", "all"):
        exp_id = f"v3_A3_gate-4_{dataset_tag}"
        model_id = f"DGmix_v3_DS_SMGP_{dataset_tag}_96_96_A3"
        run_exp(
            args.python_bin,
            exp_id,
            model_id,
            base_args,
            [*map_args, "--gate_init", "-4"],
            args.dry_run,
        )
