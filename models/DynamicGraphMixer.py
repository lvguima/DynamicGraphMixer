...
from layers.TemporalEncoder import TemporalEncoderTCN, TemporalEncoderSSM, TemporalEncoderTransformer
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        temporal_encoder = getattr(configs, "temporal_encoder", "tcn")
        temporal_encoder = temporal_encoder.lower()
        self.encoder_uses_patch = self.use_patch and temporal_encoder in {"ssm", "transformer"}
        if temporal_encoder == "ssm":
            ssm_state_dim = int(getattr(configs, "ssm_state_dim", 0))
            if ssm_state_dim <= 0:
                ssm_state_dim = configs.d_ff
            self.temporal_encoder = TemporalEncoderSSM(
    ...
    def forecast(self, x_enc):
        ...
        if self.use_patch and not self.encoder_uses_patch:
