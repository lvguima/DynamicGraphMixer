# Run: python plot/plot_predlen_errors.py --dataset flotation --exp_prefix v8_predlen_flotation --graph_log_dir ./graph_logs --out_dir ./plot
# Or:  python plot/plot_predlen_errors.py --exp_id v8_predlen_flotation_mi_pl2 --graph_log_dir ./graph_logs --out_dir ./plot
import argparse
import csv
import math
import os
...
def main():
    parser = argparse.ArgumentParser(description="Plot MAE/MSE vs pred_len.")
    ...
    plot_dir = os.path.join(args.out_dir, "predlen_errors")
    ...
    for pred_len, exp_name, stats_path in exps:
        ...
        mses = [_get_float(r, "mse") for r in rows]
        maes = [_get_float(r, "mae") for r in rows]
        label = exp_name if pred_len is None else f"pl{pred_len}"
        plt.plot(epochs, mses, marker="o", label=f"{label} MSE")
        plt.plot(epochs, maes, marker="s", label=f"{label} MAE")
    plt.xlabel("Epoch")
    plt.ylabel("Error")
    plt.title(f"{args.dataset}: MAE/MSE vs epoch")
    plt.grid(True, alpha=0.3)
    plt.legend(ncol=2, fontsize=8)
    out_path = os.path.join(plot_dir, f"{args.dataset}_epoch_errors.png")
    plt.tight_layout()
