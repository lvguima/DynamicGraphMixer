# Run: python plot/plot_predlen_prior_overlap.py --dataset flotation --exp_prefix v8_predlen_flotation --graph_log_dir ./graph_logs --out_dir ./plot
# Or:  python plot/plot_predlen_prior_overlap.py --exp_id v8_predlen_flotation_mi_pl2 --graph_log_dir ./graph_logs --out_dir ./plot
import argparse
import csv
import os
import re
...
def main():
    parser = argparse.ArgumentParser(description="Plot dyn-vs-prior similarity vs pred_len.")
    ...
    plot_dir = os.path.join(args.out_dir, "predlen_prior")
    ...
    for pred_len, exp_name, stats_path in exps:
        ...
        overlap = [_get_float(r, "dyn_vs_prior_overlap") for r in rows]
        l1 = [_get_float(r, "dyn_vs_prior_l1") for r in rows]
        label = exp_name if pred_len is None else f"pl{pred_len}"
        axes[0].plot(epochs, overlap, marker="o", label=label)
        axes[1].plot(epochs, l1, marker="s", label=label)
    axes[0].set_xlabel("Epoch")
    axes[0].set_ylabel("dyn_vs_prior_overlap")
    axes[0].set_title("Top-k Overlap (higher=closer)")
    axes[0].grid(True, alpha=0.3)

    axes[1].set_xlabel("Epoch")
    axes[1].set_ylabel("dyn_vs_prior_l1")
    axes[1].set_title("L1 Distance (lower=closer)")
    axes[1].grid(True, alpha=0.3)

    for ax in axes:
        ax.legend(fontsize=8)

    fig.suptitle(f"{args.dataset}: Dynamic vs Prior (epoch)")
    out_path = os.path.join(plot_dir, f"{args.dataset}_epoch_prior.png")
    fig.tight_layout()
