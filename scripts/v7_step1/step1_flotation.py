...
def main() -> None:
    parser = argparse.ArgumentParser(description="v7 Step1 baseline check for flotation (pred_len=96).")
    ...
    exp_id = f"v7_step1_{DATASET['name']}_96"
    model_id = f"DGmix_v7_step1_{DATASET['name']}_96"
    cmd = [
        args.python_bin,
        "run.py",
        "--model_id",
    ...
    print("Running", DATASET["name"], "v7 Step1")
    if args.dry_run:
        print(" ".join(cmd))
        return
