...
def run_one(python_bin, base_args, layers, norm, dry_run):
    exp_id = f"v7_step4_gcn_{DATASET['name']}_L{layers}_N{norm}"
    model_id = f"DGmix_v7_step4_gcn_{DATASET['name']}_L{layers}_N{norm}"
    cmd = [
        python_bin,
        "run.py",
        "--model_id",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="v7 Step4 GCN sweep for flotation (pred_len=96).")
    ...
    for layers in LAYERS_LIST:
        for norm in NORM_LIST:
            run_one(args.python_bin, base_args, layers, norm, args.dry_run)
