...
def main() -> None:
    parser = argparse.ArgumentParser(description="v7 Step5 GCN best config for flotation (pred_len=96).")
    ...
    exp_id = f"v7_step5_gcn_{DATASET['name']}_L1_Nsym"
    model_id = f"DGmix_v7_step5_gcn_{DATASET['name']}_L1_Nsym"
    cmd = [
        args.python_bin,
        "run.py",
        "--model_id",
    ...
    print("Running", DATASET["name"], "v7 Step5 GCN best")
    if args.dry_run:
        print(" ".join(cmd))
        return
